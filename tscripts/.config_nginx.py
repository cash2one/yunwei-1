                                ...
                                if type_l == 'block' and isinstance(value_l, list):
                                    for k in value_l:
                                        if isinstance(k, dict):
                                            ...
                                            if type_p == 'item' and name_p == 'proxy_pass' and isinstance(value_p, list):
                                                #if re.search(r'(http://|https:)(\w)',value_p[0])
                                                #port_locat.append([param_l,value_p[0]])
...
def write_nginx_data():
    ...
    for ngx_file in os.listdir(ngx_dir):
        if re.search(r'%s$' %math_ngx,ngx_file):
            ...
            ###返回两个字典            
            ups_location,ups_server = get_ngx_info(ngx_path) 
     
            ###循环信息字典，提前数据
            for k_us,v_us in ups_server.items():
                group_name  = k_us
                server_info = v_us
                pass_list   = ups_location.get(k_us,[])  
