...
'''
date: 2016/07/30
role: 从yunwei库中的all_server获取所有服务器执行修改密码脚本modify_passwd.py 
usage: ke_passwd.py 
2016/08/13  添加大数据服务器
'''
...
def get_host_path(host_option):
    ...
    big_sql = "SELECT mapping_ip FROM %s WHERE project_use='bigdata' AND server_ip REGEXP '10.10.[0-9]\.'" %(tb_sv,) 

    ###连接yunwei数据库
    mbs = mysqlBase(log_path,**info_sv)
    ip_query  = mbs.query(ip_sql)
    big_query = mbs.query(big_sql)
    ...
    ###临时文件名
    write_file = 'host_%s.tmp'%time_string
...
###调用ansible执行密码修改脚本函数
def tool_mod_pass(host_ansible,host_option):
    ###实例化ansible
...
#    center_manage.execAnsible('copy','src=/data/tscripts/ dest=/data/tscripts/ owner=root group=root mode=0777',host_ansible,host_option,10)
    ...
    ###调用ansible完成执行
    r_flag,r_out = center_manage.execAnsible('raw','python /data/tscripts/modify_passwd.py',host_ansible,host_option,10)
    if r_flag == 0 and isinstance(r_out, dict):
        for k,v in r_out.items():
            if not re.search(r'success',v):
                logIns.writeLog('error','have server exec modify_passwd error %s'%k)
                r_flag = 1
...
if __name__ == "__main__":
    ...
    ###定义日志标识
    logIns  = log('1017',log_path)
    logMain = log('1017','/log/yunwei/yunwei.log')
    ...
    ###调用ansible执行所有服务器上密码修改脚本
    ansi_code = ip_code
    if ip_code == 0:
        ansi_code = tool_mod_pass(ip_path,host_option)
