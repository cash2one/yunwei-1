...
def del_log(old_str,nohup_list,**log_dict):
    ###编译正则匹配模式
    pattern = re.compile(r'^(log\.log_\d{4}(-\d{1,2}){2}|catalina\.\d{4}(-\d{1,2}){2}\.out|agent\.log\.\d{4}(-\d{1,2}){2})$')
    ###遍历
...
def del_main():
    ...
    ###7天前日期时间
    date_old = date_now + datetime.timedelta(-7)
    old_str  = date_old.strftime("%y%m%d%H%M")

    ###需要遍历的目录
    log_dict = {'nodejs':'/data/webapps','tomcat':'/data/tools','agent':'/data/logs/agent'}  ###键值主要是备忘，脚本没意义

    ###清空目录
    nohup_list = ['nohup.out','uwsgi.log']
